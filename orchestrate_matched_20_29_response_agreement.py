...
LOG_DIR = ROOT / 'logs'
OUT_ROOT = ROOT / 'checkpoints_age_groups_response_agreement'
STATUS_LOG = LOG_DIR / 'matched_20_29_response_agreement.log'
DONE_FLAG = ROOT / 'results' / 'matched_20_29_response_agreement_done.txt'
...
def main():
    ...
    append('=== matched 20-29 response-agreement experiment started ===')
    caffeinate_proc = launch_caffeinate()
    output_root = OUT_ROOT / 'run_B_choice_emphasis_respagree'
    output_root.mkdir(parents=True, exist_ok=True)
    log_path = LOG_DIR / 'run_B_choice_emphasis_respagree.log'
    cmd = [
    ...
    if proc.returncode != 0:
        raise RuntimeError('Response-agreement experiment failed')
    DONE_FLAG.parent.mkdir(parents=True, exist_ok=True)
    DONE_FLAG.write_text('matched 20-29 response-agreement experiment complete\n')
    append(f'Done flag written: {DONE_FLAG}')
    append('=== matched 20-29 response-agreement experiment finished ===')
