...
LOG_DIR = ROOT / 'logs'
OUT_ROOT = ROOT / 'checkpoints_age_groups_rtshape'
STATUS_LOG = LOG_DIR / 'matched_20_29_rtshape.log'
DONE_FLAG = ROOT / 'results' / 'matched_20_29_rtshape_done.txt'
...
def main():
    ...
    append('=== matched 20-29 rt-shape experiment started ===')
    caffeinate_proc = launch_caffeinate()
    output_root = OUT_ROOT / 'run_B_rtshape'
    output_root.mkdir(parents=True, exist_ok=True)
    log_path = LOG_DIR / 'run_B_rtshape.log'
    cmd = [
    ...
    if proc.returncode != 0:
        raise RuntimeError('RT-shape run failed')
    DONE_FLAG.parent.mkdir(parents=True, exist_ok=True)
    DONE_FLAG.write_text('matched 20-29 rt-shape experiment complete\n')
    append(f'Done flag written: {DONE_FLAG}')
    append('=== matched 20-29 rt-shape experiment finished ===')
